...
class DenseCapsule(nn.Module):
    ...
    def forward(self, x):
        ...
        #print(self.weight.shape, x[:, None, :, :, None].shape)
        #print(type(self.weight.data), type(x[:, None, :, :, None].data))
        #exit()
        #x_hat = torch.squeeze(torch.matmul(self.weight.cpu(), x[:, None, :, :, None].cpu()), dim=-1).cuda()
        #self.weight.cuda()
        #x[:, None, :, :, None].cuda()
